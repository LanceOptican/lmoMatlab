function [bar_xtick,hb,he, bar_x]=errorbar_groups(bar_input,errorbar_input,varargin)
%
...
%
% bar_input, whose first dimension indicates the number of bars in a group
% and the second dimension indicates the number of groups, i.e.:
% [N_bars,N_grps]=size(bar_input);
%
% The bars are grouped together, similar to the plot produced by BAR(Y,'grouped').
%
...
%
% [..., bar_x] = ERRORBAR_GROUPS(...) returns the x-values of each bar
%
...
% [N_bars,N_grps]=size(bar_input);  % this seems backwards, so rename
% everything
[N_bars,N_grps]=size(bar_input);
bar_width=0.9;
errorbar_width=0.75;
bar_colors=distinguishable_colors(N_bars); % get some distinguishable colors for the bars!
errorbar_colors=zeros(N_bars,3); % default errorbar color is black
...
bar_names=strtrim(cellstr(num2str((1:N_grps)')));
grp_names=strtrim(cellstr(num2str((1:N_bars)')));
FigID=[];
AxID=[];
...
shift_span=(1-bar_width)*(N_bars-1);
bar_shift=linspace(-shift_span/2,+shift_span/2,N_bars);

% compute position of group x ticks
bar_xtick=N_bars/2 + 0.5:N_bars:(N_grps*N_bars) - N_bars/2+0.5;
...
for br=1:N_bars
    x =  (br:N_bars:(N_grps*N_bars)-(N_bars-br))-bar_shift(br);
    bar_x(br, :) = x;   % save bar positions for output
    hb(br)=bar( ...
        x, ... % this is the x position for each bar
        bar_input(br,:),  ... % this is the y position for each bar
        bar_width/N_bars, ... % this is the width of each bar
        'FaceColor',bar_colors(br,:), ... % color parameter
...
if any(any(errorbar_lower~=0))||any(any(errorbar_upper~=0))
    for br=1:N_bars
        ...
        he(br)=errorbar( ...
            x, ... % this is the x position for each bar
            bar_input(br,:),  ... % this is the y position for each bar
            errorbar_lower(br,:), ... % this is the error low value for each bar
            errorbar_upper(br,:), ... % this is the error high value for each bar
            'Color',errorbar_colors(br,:), ... % color parameter
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % what does this do? It screws up the errorbars,
    % so I am removing it. LM Optican
%     for grp=1:N_bars
%         if verLessThan('matlab', '8.4.0')
...
% LM Optican:  Seems not to work in R2015a
