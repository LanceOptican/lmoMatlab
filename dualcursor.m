...
% define delta label string -- LMO
strx = '\Deltax '; % '({x}_{2}-{x}_{1}) = '
stry = '\Deltay '; % '({y}_{2}-{y}_{1}) = '
strfmtx = '%+.0f';
strfmty = '%+.1f';
...
switch state
    case 'on'
        ...
        if isempty(lineh)
            ...
            % LMOptican: select line with most points and smallest peak
            ...
            lov = [];
            for k = 1:numel(lineh)
                thi = numel(lineh(k).XData);
                tlo = max(abs(lineh(k).YData));
                if thi >= hi
                    his = [his, k];
                    hiv = [hiv, thi];
                    lov = [lov, tlo];
        ...
        fsz = 12; % 8
        deltah(1) = text(deltalabelpos(1,1),deltalabelpos(1,2),[strx num2str(dx, strfmtx)], ...
            'Units','Normalized', ...
            ...
            'Interpreter','tex', ...
            'Parent',axh);      %dx
        deltah(2) = text(deltalabelpos(2,1),deltalabelpos(2,2),[stry num2str(dy,strfmty)], ...
            'Units','Normalized', ...
            'HorizontalAlignment','left', ...
            'Tag','CursorDeltaText', ...
            'FontSize',fsz, ...
    ...
    case 'move'          % Execute the WindowButtonMotionFcn
        ...
        if ~isempty(gco)
            ...
            if slide            %Move to new data value
                ...
                set(deltah(1),'String',[strx num2str(dx,strfmtx)]);
                set(deltah(2),'String',[stry num2str(dy,strfmty)]);
...
% find any line with more thatn two points
